...
public class Redis1MainConfig {
    public Redis1MainConfig(
            ...
            @NotNull List<String> nodeList
    ) {
        this.nodeList = nodeList;
    }

    // !!!application.yml 의 datasource-redis 안에 작성된 이름 할당하기!!!
    public static final @NotNull String REDIS_CONFIG_NAME = "redis1-main";

    // Redis Template Bean 이름
    public static final @NotNull String REDIS_TEMPLATE_NAME = REDIS_CONFIG_NAME + "_template";

    private final @NotNull List<String> nodeList;

    // ---------------------------------------------------------------------------------------------
    @Bean(name = REDIS_CONFIG_NAME + "_ConnectionFactory")
    public @NotNull LettuceConnectionFactory redisConnectionFactory() {
        ...
        if (nodeList.size() > 1) {
            // 클러스터 연결 설정
            @NotNull LettuceClientConfiguration clientConfig =
            ...
            @NotNull String[] hostPort = nodeList.getFirst().split(":");
            @NotNull String host = hostPort[0];
            int port = Integer.parseInt(hostPort[1]);

            // 단일 Redis 서버 연결 설정
            @NotNull RedisStandaloneConfiguration standaloneConfig = new RedisStandaloneConfiguration(host, port);
